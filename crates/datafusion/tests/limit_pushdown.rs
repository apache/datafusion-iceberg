...
mod tests {
          ...
          SortExec: TopK(fetch=3), expr=[pickup_at@1 ASC NULLS LAST], preserve_partitioning=[true]
            FilterExec: pickup_date@4 = 2024-01-10
              RepartitionExec: partitioning=RoundRobinBatch(16), input_partitions=1
                IcebergTableScan projection:[vendor_id,pickup_at,passenger_count,trip_distance,pickup_date] predicate:[pickup_date = 2024-01-10]
        ");
    ...
    async fn passes_the_scan_limit_to_the_iceberg_source() -> Result<()> {
        ...
        GlobalLimitExec: skip=0, fetch=3
          CooperativeExec
            IcebergTableScan projection:[pickup_date] predicate:[] limit:[3]
        ");
        insta::assert_snapshot!(batches, @r"
    +-------------+
    | pickup_date |
